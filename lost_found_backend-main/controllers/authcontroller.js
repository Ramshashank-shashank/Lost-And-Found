...
exports.registerUser = async (req, res) => {
  let { email, whatsappNumber, password, hostelName, name } = JSON.parse(
    req.body.userData
  ); // Parse userData from req.body
...
exports.addnewsurvey = async (req, res) => {
  ...
  if (!uid || !surveyResponse) {
    return res
      .status(400)
      .json({ message: "UID and survey response are required." });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Survey response submitted successfully!" });
  } catch (error) {
